    ...
    .map(
      (item) =>
        `<tr>
          ...
          <td style="padding:8px 12px;border-bottom:1px solid #1a2e22;text-align:right;color:${ps.color};font-weight:600;">${item.price.toFixed(2)}€</td>
        </tr>`
    )
    .join("");
...
<body style="margin:0;padding:0;background:#050505;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:40px 24px;">
    ...
    <!-- Main card -->
    <div style="background:#0e1512;border:1px solid ${ps.border};border-radius:16px;padding:32px 24px;position:relative;overflow:hidden;">
      <!-- Platform accent line -->
      <div style="position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,transparent,${ps.color},transparent);"></div>

      <!-- Platform icon + check -->
      <div style="text-align:center;margin-bottom:20px;">
        <div style="display:inline-flex;align-items:center;justify-content:center;width:56px;height:56px;border-radius:50%;background:${ps.bg};border:2px solid ${ps.border};">
          ${ps.icon || '<span style="font-size:24px;">✓</span>'}
        </div>
      </div>
      ...
      <p style="text-align:center;font-size:14px;color:#a9b5ae;margin:0 0 8px 0;">
      ...
      <!-- Timeline -->
      <div style="background:${ps.bg};border:1px solid ${ps.border};border-radius:10px;padding:16px;">
      ...
      ${promoCode ? `
      <!-- Promo code -->
      <div style="margin-top:20px;background:linear-gradient(135deg,${ps.bg},rgba(0,255,76,0.04));border:1px dashed ${ps.border};border-radius:10px;padding:20px;text-align:center;">
      ...
      <div style="margin-top:20px;text-align:center;">
        <a href="https://fanovaly.com/order/${orderId}" style="display:inline-block;padding:14px 32px;border-radius:14px;background:linear-gradient(135deg,${ps.color},${ps.color}cc);color:#000;font-weight:700;font-size:15px;text-decoration:none;">${t.trackOrder}</a>
      </div>
      ` : ""}

      <!-- Reorder link -->
      <div style="margin-top:12px;text-align:center;">
        <a href="https://fanovaly.com/${platform === 'x' ? '' : platform}" style="display:inline-block;padding:12px 28px;border-radius:14px;border:1px solid ${ps.border};background:transparent;color:${ps.color};font-weight:600;font-size:14px;text-decoration:none;">${t.reorder}</a>
      </div>
    </div>
...
const i18nFollowupDay1: Record<string, {
  ...
  fr: {
    ...
    body: (u, p) => `Les résultats pour <strong style="color:#fff;">@${u}</strong> sur ${platformLabel(p)} sont en cours de livraison. Tu devrais déjà voir une différence sur ton profil !<br><br>Envie d'aller encore plus loin ? Ajoute un boost supplémentaire pour accélérer ta croissance.`,
  ...
  en: {
    ...
    body: (u, p) => `Results for <strong style="color:#fff;">@${u}</strong> on ${platformLabel(p)} are being delivered. You should already see a difference on your profile!<br><br>Want to go even further? Add an extra boost to accelerate your growth.`,
    cta: "🚀 Order again",
  ...
  es: {
    ...
    body: (u, p) => `Los resultados para <strong style="color:#fff;">@${u}</strong> en ${platformLabel(p)} se están entregando. ¡Ya deberías ver una diferencia en tu perfil!<br><br>¿Quieres ir más lejos? Añade un boost extra para acelerar tu crecimiento.`,
    cta: "🚀 Pedir de nuevo",
  ...
  pt: {
    ...
    body: (u, p) => `Os resultados para <strong style="color:#fff;">@${u}</strong> no ${platformLabel(p)} estão sendo entregues. Você já deve ver uma diferença no seu perfil!<br><br>Quer ir mais longe? Adicione um boost extra para acelerar seu crescimento.`,
    cta: "🚀 Pedir novamente",
  ...
  de: {
    ...
    body: (u, p) => `Die Ergebnisse für <strong style="color:#fff;">@${u}</strong> auf ${platformLabel(p)} werden gerade geliefert. Du solltest bereits einen Unterschied auf deinem Profil sehen!<br><br>Möchtest du noch weiter gehen? Füge einen extra Boost hinzu, um dein Wachstum zu beschleunigen.`,
...
const i18nFollowupDelivered: Record<string, {
  ...
  fr: {
    ...
    body: (u, p) => `Les résultats pour <strong style="color:#fff;">@${u}</strong> sur ${platformLabel(p)} ont été livrés avec succès.<br><br>Va checker ton profil — tu vas voir la différence ! Et si tu veux continuer à grandir, relance un boost maintenant.`,
  ...
  en: {
    ...
    body: (u, p) => `Results for <strong style="color:#fff;">@${u}</strong> on ${platformLabel(p)} have been successfully delivered.<br><br>Go check your profile — you'll see the difference! And if you want to keep growing, launch another boost now.`,
    resultTitle: "Results",
    cta: "🚀 Boost again",
  ...
  es: {
    ...
    body: (u, p) => `Los resultados para <strong style="color:#fff;">@${u}</strong> en ${platformLabel(p)} han sido entregados con éxito.<br><br>¡Ve a revisar tu perfil — verás la diferencia! Y si quieres seguir creciendo, lanza otro boost ahora.`,
  ...
  pt: {
    ...
    body: (u, p) => `Os resultados para <strong style="color:#fff;">@${u}</strong> no ${platformLabel(p)} foram entregues com sucesso.<br><br>Vá conferir seu perfil — você vai ver a diferença! E se quiser continuar crescendo, lance outro boost agora.`,
    resultTitle: "Resultados",
  ...
  de: {
    ...
    body: (u, p) => `Die Ergebnisse für <strong style="color:#fff;">@${u}</strong> auf ${platformLabel(p)} wurden erfolgreich geliefert.<br><br>Schau dir dein Profil an — du wirst den Unterschied sehen! Und wenn du weiter wachsen möchtest, starte jetzt einen neuen Boost.`,
...
function followupShell(ps: PlatformStyle, title: string, bodyHtml: string, buttons: { label: string; href: string; primary?: boolean }[], footerText: string): string {
  const btnHtml = buttons.map((b) =>
    b.primary
      ? `<a href="${b.href}" style="display:inline-block;padding:14px 32px;border-radius:14px;background:linear-gradient(135deg,${ps.color},${ps.color}cc);color:#000;font-weight:700;font-size:15px;text-decoration:none;margin:4px;">${b.label}</a>`
      : `<a href="${b.href}" style="display:inline-block;padding:12px 28px;border-radius:14px;border:1px solid ${ps.border};background:transparent;color:${ps.color};font-weight:600;font-size:14px;text-decoration:none;margin:4px;">${b.label}</a>`
  ).join("");
...
<body style="margin:0;padding:0;background:#050505;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:40px 24px;">
    ...
    <div style="background:#0e1512;border:1px solid ${ps.border};border-radius:16px;padding:32px 24px;position:relative;overflow:hidden;">
      ...
      <h2 style="text-align:center;font-size:20px;font-weight:700;color:#fff;margin:0 0 16px 0;">${title}</h2>
      <p style="text-align:center;font-size:14px;color:#a9b5ae;margin:0 0 24px 0;line-height:1.6;">${bodyHtml}</p>
      <div style="text-align:center;">${btnHtml}</div>
    </div>
    <div style="text-align:center;margin-top:24px;">
      <p style="font-size:11px;color:#6b756f;margin:0;">${footerText}</p>
    </div>
  </div>
</body></html>`;
}
