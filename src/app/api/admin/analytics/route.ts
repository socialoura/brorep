...
export async function GET(req: NextRequest) {
  ...
  const revenue = await sql`
    SELECT COALESCE(SUM(
      CASE WHEN currency = 'usd' THEN ROUND(total_cents * ${rate}) ELSE total_cents END
  ...
  const topServices = await sql`
    SELECT 
      ...
      COUNT(*) as count,
      SUM(
        CASE WHEN o.currency = 'usd' THEN ROUND((item->>'price')::numeric * 100 * ${rate})
  ...
  const totalCost = await sql`
    SELECT COALESCE(SUM(ROUND(cost_cents * ${rate})), 0) as total
